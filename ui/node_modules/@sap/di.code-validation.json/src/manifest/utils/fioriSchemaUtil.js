...
var _ = require('lodash');
	...
	module.exports = {
        ...
        _handleAllOf: function (oSubSchema) {
            var oDefinitions = oSubSchema.definitions;
            var that = this;
            that._flatAllOfArr(oSubSchema, oDefinitions);
            _.each(oSubSchema.properties, function (oProperty) {
                that._flatAllOfArr(oProperty, oDefinitions);
            });
        },

        _flatAllOfArr: function (oVal, oDefinitions) {
            if (_.isObject(oVal) && oVal.hasOwnProperty("allOf")) {
                var aSubSchemas = oVal["allOf"];
                for (var ii = 0; ii < aSubSchemas.length; ii++) {
                    var oSubPropertySchema = aSubSchemas[ii];
                    this._replaceRefDefintionWithSchema(oSubPropertySchema, oDefinitions);
                    _.merge(oVal, oSubPropertySchema);
                }
                delete oVal["allOf"];
            }
        },
