...
function lint_json(grunt, allResults, cb_done) {
    if (jsonValidator) {
        ...
        fileUtils.listFilteredFiles(validationMetadata.getRootPath(), {
            extensions: [".json"],
            ignoredPaths: [path.join(appFolder, '..', 'node_modules'), distDir]
        }, function (files) {
            var fileResources = [];
...
module.exports = function(grunt) {
    var appFolder = grunt.config.get('dir.appFolder');
    var distDir = grunt.config.get('dir.dist');
    if (!appFolder || !distDir) {
        grunt.log.writeln('missing appFolder or dist folder information. skipping lint task');
        return;
    }
